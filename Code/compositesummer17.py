...
cyc=xr.open_dataset('all_nudge_a500_cyclone_centers.nc')
...
for a in inds: 
    ...
    #en=(np.where(np.logical_and(cyc.datetime.values==time+timedelta(hours=24),cyc.label.values==label))[0])
    ...
    files=[i for i in glob.glob('/glade/derecho/scratch/ajacobs/archive/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022/atm/hist/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022.cam.h2.*-{m}-{d}-00000.nc'.format(m=month,d=day))]
    files.sort()
    files1=[i for i in glob.glob('/glade/derecho/scratch/ajacobs/archive/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022/atm/hist/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022.cam.h1.*-{m}-{d}-00000.nc'.format(m=month,d=day))]
    files1.sort()
    temps=[]
    tpws=[]
    wind=[]
    ...
    for i,file in enumerate(files):
        ...
        for b in [0,1,2,3]:
            if file=='/glade/derecho/scratch/ajacobs/archive/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022/atm/hist/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022.cam.h2.2013-01-01-00000.nc' and b==0:
                continue
            ...
            plev=np.array([300])
            #print(theta_pres.shape)
            if not theta_pres.any():
                continue
    ...
    psl_mean=np.mean(np.array(psls),axis=0)/100
    ...
    file='/glade/derecho/scratch/ajacobs/archive/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022/atm/hist/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022.cam.h2.{y}-{m}-{d}-00000.nc'.format(y=year,m=month,d=day)
    fv2=xr.open_dataset(file)
    file1='/glade/derecho/scratch/ajacobs/archive/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022/atm/hist/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022.cam.h1.{y}-{m}-{d}-00000.nc'.format(y=year,m=month,d=day)
    fv1=xr.open_dataset(file1)
    hour=int(int(time.hour)/6)
    if file=='/glade/derecho/scratch/ajacobs/archive/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022/atm/hist/release-cesm2.2.1_FHIST_f09_g17_L32_nudge_at_2013_2022.cam.h2.2013-01-01-00000.nc' and hour==0:
        continue
    pressure=(fv1.hyam*fv1.P0+fv1.hybm*fv2.PS)/100
    campressl=pressure.transpose("time", "lev", "lat", "lon") # hybrid coord press levels
    ilev=fv1.ilev
    ...
    for i in range(len(x)):
        ...
        if ydiff[i]:
            y[i] = -y[i]
    start=(np.where(np.logical_and(cyc.datetime.values==slp.time.values-timedelta(hours=1),cyc.label.values==label))[0])
    end=(np.where(np.logical_and(cyc.datetime.values==slp.time.values+timedelta(hours=1),cyc.label.values==label))[0])
    if True:
        if not start:
            start_lat = clat
            start_lon = clon
...
# NW quadrant
    ...
    # SW quadrant
        ...
        if np.logical_and(np.mean(np.array(center_section)) > np.mean(np.array(non_center_section)),(slp[ind_y][ind_x]-temp_mean[ind_y][ind_x])>0):
            warm_core.append([list1,list2,list3,list6,list9,list12,list15,list18,list21,list24])
        else:
            cold_core.append([list1,list2,list3,list6,list9,list12,list15,list18,list21,list24])
with open('mininds20.pkl', 'wb') as file:
    # Dump the data into the file
    pickle.dump(mininds, file)
with open('mincoldcore20_updated.pkl', 'wb') as file:
    # Dump the data into the file
    pickle.dump(cold_core, file)
with open('minwarmcore20_updated.pkl', 'wb') as file:
    # Dump the data into the file
    pickle.dump(warm_core, file)
...
plt.savefig('fv_20_min_150_cold_test_north_temp_updated.png')
...
plt.savefig('fv_20_min_150_cold_test_north_tpw_updated.png')
fig, ax = plt.subplots()
...
plt.savefig('fv_20_min_150_cold_test_north_u10_updated.png')
fig, ax = plt.subplots()
...
plt.savefig('fv_20_min_150_cold_test_north_psl_updated.png')
fig, ax = plt.subplots()
nan_mask=np.isnan(np.array(list_7))
H, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y], weights = np.array(list_7)[~nan_mask])
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-5,vmax=5,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='m/s',extend='both')
plt.savefig('fv_20_min_150_cold_test_north_u850_updated.png')
fig, ax = plt.subplots()
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-20,vmax=20,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='K',extend='both')
plt.savefig('fv_20_min_150_cold_test_north_pvu_updated.png')
fig, ax = plt.subplots()
...
plt.imshow(H.T, origin='lower',  cmap='BrBG',vmin=-3,vmax=3,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='m/s',extend='both')
plt.savefig('fv_20_min_150_cold_test_north_pvq_updated.png')
fig, ax = plt.subplots()
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-5,vmax=5,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='m/s',extend='both')
plt.savefig('fv_20_min_150_cold_test_north_upv_updated.png')
'''fig, ax = plt.subplots()
H, xedges, yedges = np.histogram2d(np.array(list_1),np.array(list_2), bins = [x, y], weights = np.array(list_6))
H_counts, xedges, yedges = np.histogram2d(np.array(list_1),np.array(list_2), bins = [x, y])
H = H/H_counts
...
list9=[row[8] for row in warm_core]
list10=[row[9] for row in warm_core]
print(len(list3))
list_1 = [item for row in list1 for item in row]
list_2 = [item for row in list2 for item in row]
list_3 = [item for row in list3 for item in row]
...
plt.imshow(H.T, origin='lower',  cmap='bwr',vmin=-5,vmax=5,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='[K]',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_temp_updated.png')
...
plt.imshow(H.T, origin='lower',  cmap='BrBG',vmin=-3,vmax=3,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label=r'[kg/m$^2$]',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_tpw_updated.png')
fig, ax = plt.subplots()
H, xedges, yedges = np.histogram2d(np.array(list_1),np.array(list_2), bins = [x, y], weights = np.array(list_5))
H_counts, xedges, yedges = np.histogram2d(np.array(list_1),np.array(list_2), bins = [x, y])
H = H/H_counts
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-5,vmax=5,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='m/s',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_u10_updated.png')
fig, ax = plt.subplots()
H, xedges, yedges = np.histogram2d(np.array(list_1),np.array(list_2), bins = [x, y], weights = np.array(list_6))
H_counts, xedges, yedges = np.histogram2d(np.array(list_1),np.array(list_2), bins = [x, y])
H = H/H_counts
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-20,vmax=20,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='hPa',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_psl_updated.png')
fig, ax = plt.subplots()
nan_mask=np.isnan(np.array(list_8))
H, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y], weights = np.array(list_8)[~nan_mask])
H_counts, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y])
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-20,vmax=20,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='K',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_pvu_updated.png')
fig, ax = plt.subplots()
nan_mask=np.isnan(np.array(list_9))
H, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y], weights = np.array(list_9)[~nan_mask])
H_counts, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y])
...
plt.imshow(H.T, origin='lower',  cmap='BrBG',vmin=-3,vmax=3,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='m/s',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_pvq_updated.png')
fig, ax = plt.subplots()
nan_mask=np.isnan(np.array(list_10))
H, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y], weights = np.array(list_10)[~nan_mask])
H_counts, xedges, yedges = np.histogram2d(np.array(list_1)[~nan_mask],np.array(list_2)[~nan_mask], bins = [x, y])
...
plt.imshow(H.T, origin='lower',  cmap='PRGn',vmin=-5,vmax=5,
            extent=[xedges[0], xedges[-1], yedges[0], yedges[-1]])
plt.colorbar(label='m/s',extend='both')
plt.savefig('fv_20_min_150_warm_test_north_upv_updated.png')
#print(list3[0])
